...
_EXECUTOR_STATUS: dict[str, tuple[str, str]] = {
    ...
    "parse_departure_text": (
        "implemented",
        "src/sop_hub/sop/departure_text_parser.py",
    ),
    "classify_message": (
        "implemented",
    ...
    "extract_release_notice_json": (
        "implemented",
        "src/sop_hub/runner.py (OCR → JSON pipeline)",
    ),
    "identify_project": (
        "implemented",
        "src/sop_hub/sop/monitoring_plan_matcher.py (_fallback_alignment_match)",
    ),
    "create_release_batch": (
        "implemented",
        "src/sop_hub/data_agent/agent.py (ingest_release_batch)",
    ),
    "update_release_batch_fields": (
        "implemented",
        "src/sop_hub/data_agent/agent.py (ON CONFLICT DO UPDATE)",
    ),
    "query_release_batches": (
        "implemented",
        "src/sop_hub/data_agent/agent.py (batch_key lookup)",
    ),
    # ── freight detail ──
    "extract_freight_detail": (
        "implemented",
        "src/sop_hub/sop/freight_detail_extractor.py",
    ),
    "enrich_release_batch": (
        "implemented",
        "src/sop_hub/sop/enrich_release_batch.py",
    ),
    # ── departure → 95306 query ──
    "build_time_window": (
        "implemented",
        "src/sop_hub/sop/shipment_query_window.py (QueryWindow.from_reference)",
    ),
    "query_95306_waybills": (
        "implemented",
        "src/sop_hub/sop/query_95306_shipments.py (query_95306_shipments_by_window)",
    ),
    # ── tracking / shipment status sync ──
    "poll_shipment_snapshots": (
        "implemented",
        "src/sop_hub/sop/shipment_status_sync.py (ShipmentStatusSync.sync)",
    ),
    # ── departure → 95306 query → create wagons ──
    "create_wagon_shipments": (
        "implemented",
        "src/sop_hub/sop/create_wagon_shipments.py",
    ),
    "bind_wagons_to_release_batch": (
        "implemented",
        "src/sop_hub/sop/create_wagon_shipments.py (create_wagon_shipments_from_candidates)",
    ),
    "check_existing_wagon_shipments": (
        "implemented",
        "src/sop_hub/sop/create_wagon_shipments.py (idempotent skip + conflict check)",
    ),
    # ── prototype / dry_run_only ──
    "generate_departure_excel_task": (
        "implemented",
        "src/sop_hub/sop/departure_excel.py (generate_departure_excel → .xlsx from SOP template)",
    ),
    "generate_factory_transport_json_task": (
        "implemented",
        "src/sop_hub/sop/factory_upload.py (build_upload_payloads → JSON payload)",
    ),
    # ── factory upload ──
