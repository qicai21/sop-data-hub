...
from sop_hub.pipeline.models import ClassificationResult
...
class BusinessGroupImageClassifier:
    ...
    def classify(
        ...
        *,
        ...
        if category == "检装车通知单":
            # #issue-20260619 标题硬锚(掉了的那道闸,补回来):真检装车单标题必含
            # “检(、)装车通知单”。VLM 易把同版式的日现场工作记录表/请车表(印刷表
            # + 到站/船名/品名/道线列)按版式误判成检装车单,prompt 又鼓励“60%视觉
            # 证据即输出”。这里做**确定性闸**:detected_title 归一后不含“检装车通知单”
            # → 降级 other,绝不进检装车抽取链(2026-06-19 日报误判根因)。
            _t = (detected_title or "")
            for _c in ("、", " ", "，", ",", "　"):
                _t = _t.replace(_c, "")
            if "检装车通知单" not in _t:
                category = "other"
