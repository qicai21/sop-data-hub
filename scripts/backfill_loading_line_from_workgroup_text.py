...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill wagon_shipments.loading_line from 铁晟业务工作群 history"
    )
    parser.add_argument(
        "--db",
    ...
    parser.add_argument(
        "--projects",
        default="chaoyang_steel,jilin_jingang_jinzhou",
        help="Comma-separated project ids",
    )
    parser.add_argument(
        "--group-name",
        default="铁晟业务工作群",
        help="Workgroup name to scan",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    try:
        decisions = decide_backfill(conn, project_ids=project_ids, group_name=args.group_name)
        ...
        for decision in decisions:
            g = decision.group
            print(
                f"{decision.status}\t{g.project_id}\t{g.dispatch_train_code}\t"
