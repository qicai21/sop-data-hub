...
2. **空字段铁律**：如果该单元格在图中是空白的，**必须输出空字符串 ""**。**严禁**根据上下文（如同一张单子前几行有"鞍子河"）来推断填充。检装车通知单的"收货人/船名/到站"列通常只在表头段(开头 1-4 行)有内容，后续的真车号行该列**应当为空**，请如实输出空串。
3. **车号 7 位强制**：car_no 字段必须是恰好 7 位数字字符串。如果你识别出的车号不足 7 位（如 6 位）或多于 7 位，说明识别错了，请重新仔细看该格——铁路车号在此表格中只可能是 7 位，不存在 6 位或 8 位。
4. **零遗漏**：严禁跳过任何一行，只要有序号就必须输出。
5. **划线检测**：如果该行被划掉，设置 "defect": true。
6. **输出要求**：只输出 JSON 数组，严禁任何解释。"""
...
4. **空字段铁律**：单元格在图中为空时，输出 ""，**不要**根据上下文推断填充（如同一列前几行有"鞍子河"，后面的真车号行该列应为空）。
5. **车号必须 7 位**：car_no 是恰好 7 位数字字符串。若识别到 6 位或 8 位，重新仔细看——铁路车号在此表只可能 7 位。"""
