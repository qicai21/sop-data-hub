...
RunMode = Literal["dry_run", "commit"]


@dataclass(frozen=True)
class CandidateFinalizeSummary:
    candidate_id: str
...
class InspectionFinalizeResult:
    ...
    candidates: list[CandidateFinalizeSummary] = field(default_factory=list)
...
def finalize_inspection_candidates(
    *,
...
) -> InspectionFinalizeResult:
    """Finalize SOP-authorized inspection candidates into formal shipment matches.
    ...
    is intentionally explicit and idempotent: dry_run never writes, commit writes
    via ON CONFLICT(release_batch_id, shipment_ydid) upsert and then recomputes
    release_batches.actual_wagon_count from the formal table.
    """
    if run_mode not in ("dry_run", "commit"):
        raise ValueError("run_mode must be dry_run or commit")
    ...
    try:
        ...
        summaries: list[CandidateFinalizeSummary] = []
        ...
        satisfied = bool(planned_rows) and not any(item["reason"] in {"first-car-not-found", "db-window-empty"} for item in excluded)
        reason = "ready-to-commit" if satisfied else "no-confirmed-95306-rows"

        committed_count = 0
        formal = _formal_summary(rail, release_batch_id)
        actual_wagon_count = formal["count"]
        if run_mode == "commit" and satisfied:
...
def _failure(run_mode: RunMode, release_batch_id: str, project_id: str, operator_note: str, reason: str) -> InspectionFinalizeResult:
    return InspectionFinalizeResult(run_mode, release_batch_id, project_id, operator_note, False, reason)
            ...
            excluded.append(
                {
                    ...
                    "reason": "ocr-defect" if row.get("defect") else "outside-target-release-segment",
                }
            )
...
def _build_formal_rows_for_candidate(
    ...
    *,
    ...
    window_minutes: int,
) -> tuple[list[dict[str, Any]], CandidateFinalizeSummary, list[dict[str, Any]]]:
    excluded: list[dict[str, Any]] = []
    first_car = _first_car(inspection_rows)
    empty_summary = CandidateFinalizeSummary(candidate_id, source_file_name, len(inspection_rows), 0, 0.0, first_car)
    if not first_car:
    ...
    summary = CandidateFinalizeSummary(candidate_id, source_file_name, len(inspection_rows), len(formal_rows), planned_weight, first_car, start, end)
...
def _formal_row_from_shipment(spec: Any, candidate_id: str, source_file_name: str, ocr_row: Mapping[str, Any], shipment: sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        ...
        "match_rule": f"finalize-inspection candidate={candidate_id}; release MatchSpec station={spec.station_aliases}; cargo={spec.cargo_aliases}; ticketed_at ±30m; OCR segment split; DB car number authoritative",
    }
